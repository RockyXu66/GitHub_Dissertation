...
vector<unsigned char*> PCA_::reconstruct(int imageIndex, unsigned char* newImage, unsigned char* smoothedNewImage, Mat bgr[3], int cell_num, int x, int y, bool isBlur, int kernelSize){
...
//    cout<<"===Start reconstruction==="<<endl;
    ...
    // Smooth the edge of two cell images
    Mat smoothedResultImage= resultImage.clone();
    Mat smoothedImage = resultImage.clone();
    medianBlur (smoothedImage, smoothedImage, kernelSize);
    for(int i=1; i<smoothedResultImage.rows-1; i++){
        for(int j=1; j<smoothedResultImage.cols-1; j++){
            if(j%cell_dimension==0){
                smoothedResultImage.at<Vec3b>(i, j-1) = smoothedImage.at<Vec3b>(i, j-1);
                smoothedResultImage.at<Vec3b>(i, j) = smoothedImage.at<Vec3b>(i, j);
                smoothedResultImage.at<Vec3b>(i, j+1) = smoothedImage.at<Vec3b>(i, j+1);
                smoothedResultImage.at<Vec3b>(i, j+2) = smoothedImage.at<Vec3b>(i, j+2);
            }
            if(i%cell_dimension==0){
                smoothedResultImage.at<Vec3b>(i-1, j) = smoothedImage.at<Vec3b>(i-1, j);
                smoothedResultImage.at<Vec3b>(i, j) = smoothedImage.at<Vec3b>(i, j);
                smoothedResultImage.at<Vec3b>(i+1, j) = smoothedImage.at<Vec3b>(i+1, j);
                smoothedResultImage.at<Vec3b>(i+2, j) = smoothedImage.at<Vec3b>(i+2, j);
            }
        }
    }
    ...
    for (int x = 0; x<image_width; x++) {
        for(int y=0; y<image_height; y++){
            ...
            smoothedNewImage[(y+x*image_height) * 3] = smoothedResultImage.at<cv::Vec3b>(x,y)[2];
            smoothedNewImage[(y+x*image_height) * 3 + 1] = smoothedResultImage.at<cv::Vec3b>(x,y)[1];
            smoothedNewImage[(y+x*image_height) * 3 + 2] = smoothedResultImage.at<cv::Vec3b>(x,y)[0];
        }
    }
    if(isBlur){
        twoImages.push_back(smoothedNewImage);
    }else{
        twoImages.push_back(newImage);
    }
    twoImages.push_back(smoothedNewImage);
