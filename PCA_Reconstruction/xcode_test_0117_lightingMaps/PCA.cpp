...
unsigned char* PCA::CalculateEigen(vector<unsigned char*> images, int width, int height) {
    ...
    fileName = "eigenVectors_cell/EigenVector_Red_" + to_string(localTime->tm_hour)
    ...
    time(&currentTime);                   // Get the current time
    localTime = localtime(&currentTime);
    fileName = "eigenVectors_cell/EigenVector_Green_" + to_string(localTime->tm_hour)
    + to_string(localTime->tm_min) + to_string(localTime->tm_sec) + ".txt";
    ...
    time(&currentTime);                   // Get the current time
    localTime = localtime(&currentTime);
    fileName = "eigenVectors_cell/EigenVector_Blue_" + to_string(localTime->tm_hour)
    + to_string(localTime->tm_min) + to_string(localTime->tm_sec) + ".txt";
...
//    }
    ...
    meanFileName = "eigenVectors_cell/mean_Red.txt";
    scoresFileName = "eigenVectors_cell/scores40_Red.txt";
    vector<MatrixXd> ReconstructedData_Red = WholeReconstruction(data_r_cells, fileName, meanFileName, scoresFileName, imageIndex);
    fileName = "eigenVectors_cell/highEigen40_Green.txt";
    meanFileName = "eigenVectors_cell/mean_Green.txt";
    scoresFileName = "eigenVectors_cell/scores40_Green.txt";
    vector<MatrixXd> ReconstructedData_Green = WholeReconstruction(data_g_cells, fileName, meanFileName, scoresFileName, imageIndex);
    fileName = "eigenVectors_cell/highEigen40_Blue.txt";
    meanFileName = "eigenVectors_cell/mean_Blue.txt";
    scoresFileName = "eigenVectors_cell/scores40_Blue.txt";
    vector<MatrixXd> ReconstructedData_Blue = WholeReconstruction(data_b_cells, fileName, meanFileName, scoresFileName, imageIndex);
    ...
    // Save pixel value
    ofstream PCAImageFile_Red, PCAImageFile_Green, PCAImageFile_Blue;
    PCAImageFile_Red.open("PCAImages/PCAImages_Red.txt", ios::app);
    PCAImageFile_Green.open("PCAImages/PCAImages_Green.txt", ios::app);
    PCAImageFile_Blue.open("PCAImages/PCAImages_Blue.txt", ios::app);
    for (int i = 0; i < width*height; i++) {
        PCAImageFile_Red << int(data_r_R(0, i)) << " ";
        PCAImageFile_Green << int(data_g_R(0, i)) << " ";
        PCAImageFile_Blue << int(data_b_R(0, i)) << " ";
    }
    PCAImageFile_Red.close();
    PCAImageFile_Green.close();
    PCAImageFile_Blue.close();
    cout<<"Save PCA image success"<<endl;
...
vector<MatrixXd> PCA::WholeReconstruction(vector<MatrixXd> whole_data, string fileName, string meanFileName, string scoresFileName, int imageIndex){
