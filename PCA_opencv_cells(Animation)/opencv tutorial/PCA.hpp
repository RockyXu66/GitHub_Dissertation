...
public:
    ...
    Mat CalculateEigen(vector<Mat> images, int image_num, int width, int height);
    
    unsigned char* Mean(MatrixXd data_r, MatrixXd data_g, MatrixXd data_b, unsigned char* meanImage);
    
    void Eigen(MatrixXd r_data, MatrixXd g_data, MatrixXd b_data, string name, string fileName);
