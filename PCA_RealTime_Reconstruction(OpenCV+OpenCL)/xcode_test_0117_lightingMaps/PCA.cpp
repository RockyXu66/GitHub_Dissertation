...
unsigned char* PCA_::reconstruct(int imageIndex, unsigned char* newImage, Mat bgr[3], int cell_num, int x, int y, bool isBlur){
    ...
    int NUM_VALUES = 512;//cell_dimension*cell_dimension;
...
//    cout<<"===Start reconstruction==="<<endl;
    ...
    vector<vector<float*>> cl_eigenvectors_b, cl_eigenvectors_g, cl_eigenvectors_r;
    vector<float*> eigenvectors_b, eigenvectors_g, eigenvectors_r;
    for(int m=0; m<x; m++){
        for(int n=0; n<y; n++){
            int index = n+m*y;
            for(int i=0; i<num_components; i++){
                float* eigenvector_b = (float*)malloc(sizeof(cl_float)*(NUM_VALUES));
                float* eigenvector_g = (float*)malloc(sizeof(cl_float)*(NUM_VALUES));
                float* eigenvector_r = (float*)malloc(sizeof(cl_float)*(NUM_VALUES));
                for(int j=0; j<NUM_VALUES; j++){
                    if(j<cell_dimension*cell_dimension){
                        eigenvector_b[j] = pcas_b[index].eigenvectors.row(i).at<float>(0, j);
                        eigenvector_g[j] = pcas_g[index].eigenvectors.row(i).at<float>(0, j);
                        eigenvector_r[j] = pcas_r[index].eigenvectors.row(i).at<float>(0, j);
                    }else{
                        eigenvector_b[j] = 0;
                        eigenvector_g[j] = 0;
                        eigenvector_r[j] = 0;
                ...
                eigenvectors_b.push_back(eigenvector_b);
                eigenvectors_g.push_back(eigenvector_g);
                eigenvectors_r.push_back(eigenvector_r);
            }
            cl_eigenvectors_b.push_back(eigenvectors_b);
            cl_eigenvectors_g.push_back(eigenvectors_g);
            cl_eigenvectors_r.push_back(eigenvectors_r);
        }
    }
...
//            Mat r = pcas_r[index].backProject(scoreR).reshape(1,cell_dimension);
            ...
            // Use OpenCL to speed up
            float* test_out_b = (float*)malloc(sizeof(cl_float) * NUM_VALUES);
            float* test_out_g = (float*)malloc(sizeof(cl_float) * NUM_VALUES);
            float* test_out_r = (float*)malloc(sizeof(cl_float) * NUM_VALUES);
            void* mem_out_b = gcl_malloc(sizeof(cl_float) * NUM_VALUES, NULL, CL_MEM_WRITE_ONLY);
            void* mem_out_g = gcl_malloc(sizeof(cl_float) * NUM_VALUES, NULL, CL_MEM_WRITE_ONLY);
            void* mem_out_r = gcl_malloc(sizeof(cl_float) * NUM_VALUES, NULL, CL_MEM_WRITE_ONLY);
            float* score_b = (float*)malloc(sizeof(cl_float)*1);
            float* score_g = (float*)malloc(sizeof(cl_float)*1);
            float* score_r = (float*)malloc(sizeof(cl_float)*1);
            dispatch_sync(queue, ^{
                size_t wgs;
                gcl_get_kernel_block_workgroup_info(addVector_kernel, CL_KERNEL_WORK_GROUP_SIZE, sizeof(wgs), &wgs, NULL);
                cl_ndrange range = {1, {0, 0, 0}, {static_cast<size_t>(NUM_VALUES), 0, 0}, {wgs, 0, 0}};
                for(int k=0; k<num_components; k++){
                    score_b[0] = scoreB.at<float>(0,k);
                    score_g[0] = scoreG.at<float>(0,k);
                    score_r[0] = scoreR.at<float>(0,k);
                    void* mem_eigenvector_b  = gcl_malloc(sizeof(cl_float) * NUM_VALUES, cl_eigenvectors_b[index][k],
                                                          CL_MEM_READ_ONLY | CL_MEM_COPY_HOST_PTR);
                    void* mem_eigenvector_g  = gcl_malloc(sizeof(cl_float) * NUM_VALUES, cl_eigenvectors_g[index][k],
                                                          CL_MEM_READ_ONLY | CL_MEM_COPY_HOST_PTR);
                    void* mem_eigenvector_r  = gcl_malloc(sizeof(cl_float) * NUM_VALUES, cl_eigenvectors_r[index][k],
                                                          CL_MEM_READ_ONLY | CL_MEM_COPY_HOST_PTR);
                    void* mem_score_b  = gcl_malloc(sizeof(cl_float) * 1, score_b,
                                               CL_MEM_READ_ONLY | CL_MEM_COPY_HOST_PTR);
                    void* mem_score_g  = gcl_malloc(sizeof(cl_float) * 1, score_g,
                                               CL_MEM_READ_ONLY | CL_MEM_COPY_HOST_PTR);
                    void* mem_score_r  = gcl_malloc(sizeof(cl_float) * 1, score_r,
                                               CL_MEM_READ_ONLY | CL_MEM_COPY_HOST_PTR);
                    addVector_kernel(&range,(cl_float*)mem_out_b, (cl_float*)mem_score_b, (cl_float*)mem_eigenvector_b);
                    addVector_kernel(&range,(cl_float*)mem_out_g, (cl_float*)mem_score_g, (cl_float*)mem_eigenvector_g);
                    addVector_kernel(&range,(cl_float*)mem_out_r, (cl_float*)mem_score_r, (cl_float*)mem_eigenvector_r);
                }
...
//                addMean_kernel(&range,(cl_float*)mem_out, (cl_float*)mem_mean);
                ...
                gcl_memcpy(test_out_b, mem_out_b, sizeof(cl_float) * NUM_VALUES);
                gcl_memcpy(test_out_g, mem_out_g, sizeof(cl_float) * NUM_VALUES);
                gcl_memcpy(test_out_r, mem_out_r, sizeof(cl_float) * NUM_VALUES);
