...
unsigned char* PCA_::CalculateEigen(vector<unsigned char*> images, int image_num, int width, int height) {
...
//    return meanImage;
    ...
    unsigned char* newImage = images[3];
    ...
    for (int i = 0; i < (width*height)/(20*20); i++) {
        ...
        Eigen(data_r_cells[i], "Red", fileName);
    }
    ...
    unsigned char* eigenTexture = images[3];
...
void PCA_::Eigen(MatrixXd data, string name, string fileName) {
    cout << name << endl;
    //cout<<data<<endl;
    MatrixXd mean = data.colwise().mean();
