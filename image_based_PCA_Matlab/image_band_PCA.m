function comp_ratio=image_band_PCA(directory,test_directory,dist_dir,file_pre,dataset,threshold,num_bands)
max_num_bands=200;
uniform=1;
...
br=num_bands-j+1;
if (uniform==1)
    band=find((sample_mean>(min_m+(max_m-min_m)*(j-1)/num_bands) & sample_mean<=(min_m+(max_m-min_m)*j/num_bands)));
else
if j==1
    band=find((sample_mean>(min_m) & sample_mean<=(min_m+(max_m-min_m)/(br))));
else
    band=find((sample_mean>(min_m+(max_m-min_m)/(br+1)) & sample_mean<=(min_m+(max_m-min_m)/(br))));
end;
end;
variab(j)=((sum(sample_var(band))+sum((sample_mean(band)-mean(sample_mean(band))).^2))/length(band));
save([dist_dir '\image_band_' num2str(num_bands) '_' num2str(N) '_' num2str(j) '.mat'],'band');
end; variab=variab/sum(variab);
toc
for j=1:num_bands
