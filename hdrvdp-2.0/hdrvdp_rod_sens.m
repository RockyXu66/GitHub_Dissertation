...
ss = la>peak_l; 
S(ss) = exp( -abs(log10(la(ss)/peak_l)).^high_exp/high_s )*rod_sens;
S(~ss) = exp( -abs(log10(la(~ss)/peak_l)).^low_exp/low_s )*rod_sens;

S = S * 10.^metric_par.csf_sr_par(end); %TODO: check if this is really needed
