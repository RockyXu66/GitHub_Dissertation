...
elseif( isfield( metric_par, 'rgb_display' ) )
    [~, IMG_E] = load_spectral_resp( sprintf( 'emission_spectra_%s.csv', metric_par.rgb_display ) );
elseif( img_channels == 1 )
    [~, IMG_E] = load_spectral_resp( 'd65.csv' );
else
...
[B_T L_adapt_test band_freq] = hdrvdp_visual_pathway( test, 'test', metric_par );
[B_R L_adapt_reference] = hdrvdp_visual_pathway( reference, 'reference', metric_par );
...
for b=1:b_count
    CSF(:,b) = hdrvdp_csf( band_freq(b), csf_la', metric_par );
end
...
for b = 1:b_count
    ...
    CSF_b = interp1( csf_log_la, CSF(:,b), clamp( imresize(log_La,get_band_size(B_T,b,1)), csf_la(1), csf_la(end) ) );
    
    % correct activity for this particular band
    %    band_mask_xc = mask_xc - mask_xo;
    ...
    for o=1:B_T.sz(b)
        ...
        band_diff = get_band(B_T,b,o) - get_band(B_R,b,o);
...
if( metric_par.do_spatial_pooling )
    S_map = sum(S_map(:))/max(S_map(:))*S_map;
end
