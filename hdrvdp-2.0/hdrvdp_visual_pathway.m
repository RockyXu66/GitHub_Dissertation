function [bands, L_adapt, band_freq] = hdrvdp_visual_pathway( img, name, metric_par )
...
[pn_Y{1} pn_jnd{1}] = build_jndspace_from_S( log10(c_l), s_C );
[pn_Y{2} pn_jnd{2}] = build_jndspace_from_S( log10(c_l), s_R );

pn_jnd{1} = pn_jnd{1} * metric_par.sensitivity_correction;
pn_jnd{2} = pn_jnd{2} * metric_par.sensitivity_correction;
...
for k=[1:2 4] % ignore S - does not influence luminance   
    ...
    P_LMR(:,:,ii) = pointOp( log10( clamp(R_LMSR(:,:,k), 10^pn_Y{ph_type}(1), 10^pn_Y{ph_type}(end)) ), ...
        pn_jnd{ph_type}, pn_Y{ph_type}(1), pn_Y{ph_type}(2)-pn_Y{ph_type}(1), 0 );
end
...
band_freq = (0.5*metric_par.pix_per_deg) .* 2.^-(0:(spyrHt( bands.pind )+1));

% CSF-filter the base band
L_mean = mean( L_adapt(:) );
    
BB = pyrBand( bands.pyr, bands.pind, sum(bands.sz) );
rho_bb = create_cycdeg_image( size(BB)*2, 1.4 * band_freq(end)*2 );
...
bands.pyr(pyrBandIndices(bands.pind,sum(bands.sz))) = fast_conv_fft( BB, csf_bb, pad_value );
